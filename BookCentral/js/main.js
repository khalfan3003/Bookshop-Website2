...
// to pick a random book and show it on screen we use random to first pick a random genre
function trendingBooks(){
  // after selecting a random number from 1 to 5 we use switch to choose a genre
  var randomGenre = Math.floor(Math.random() * 5);
  switch(randomGenre) {
    case 0:
      var genre = fantasyBooks;
      break;
    case 1:
      var genre = horrorBooks;
      break;
    case 2:
      var genre = mysteryBooks;
      break;
    case 3:
      var genre = scienceFictionBooks;
      break;
    default:
      genre = historyBooks;
      break
  ...
  // we select another random number based on the numbers of book in a genre and select a book, fetch its cover and assign it to the book cover in main page
  var randomBook = Math.floor(Math.random() * genre.length);
  cover.src = genre[randomBook].cover;
}

var cover = document.querySelector("#book_cover");
var book_name = document.querySelector("#book_name");

setInterval(trendingBooks,5000);


// to go to account or cart page
// loading variables
const bagButton = document.querySelector("#bag_btn");
